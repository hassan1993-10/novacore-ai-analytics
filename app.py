...
def render_chart(df: pd.DataFrame, user_question: str = "") -> bool:
    ...
    arabic = is_arabic(q)
    metric_label = _human_label(y)

    if px is None:
        if time_dims:
            st.line_chart(chart_df.set_index(time_dims[0])[y])
        elif dims:
            st.bar_chart(chart_df.set_index(dims[0])[y])
        return True
    ...
    if time_dims:
        ...
        # If the verified result legitimately contains one extra requested dimension,
        # show multiple series only when the number of categories is readable.
        color = None
        if dims and chart_df[dims[0]].nunique(dropna=True) <= 6:
            color = dims[0]

        title = (
            f"اتجاه {_human_label(y)}"
            if arabic else
            f"{_human_label(y)} trend"
        )
        fig = px.line(
            chart_df, x=x, y=y, color=color, markers=True,
            title=title,
            color_discrete_sequence=[brand_blue, brand_cyan, "#7B61FF", "#18B36B", "#F59E0B", "#7C8AA0"],
        )
        ...
        x = dims[0] if dims else columns[0]
        # Ranking / long categories -> horizontal bars.
        horizontal = len(chart_df) > 7 or chart_df[x].astype(str).str.len().mean() > 14
        chart_df = chart_df.sort_values(y, ascending=not horizontal)

        title = (
            f"مقارنة {_human_label(y)} حسب {_human_label(x)}"
            if arabic else
            f"{_human_label(y)} by {_human_label(x)}"
        )

        if horizontal:
            chart_df = chart_df.sort_values(y, ascending=True)
            fig = px.bar(
                chart_df, x=y, y=x, orientation="h", title=title,
                color_discrete_sequence=[brand_blue],
                text=chart_df[y].map(_short_number),
            )
        else:
            fig = px.bar(
                chart_df, x=x, y=y, title=title,
                color_discrete_sequence=[brand_blue],
                text=chart_df[y].map(_short_number),
            )
    ...
    fig.update_layout(
        ...
        plot_bgcolor="#FFFFFF",
        font=dict(size=12, color=text, family="Segoe UI, Arial"),
        title_font=dict(size=16, color="#0B1F3A"),
        xaxis_title=_human_label(x),
        yaxis_title=_human_label(y),
        showlegend=True if time_dims and dims else False,
        hovermode=hovermode,
        bargap=.28,
    )
    fig.update_xaxes(showgrid=False, zeroline=False, linecolor="#DDE6F0", tickfont=dict(color="#667085"))
    fig.update_yaxes(showgrid=True, gridcolor=grid, zeroline=False, tickformat=",", tickfont=dict(color="#667085"))
...
def render_supporting_data(df: pd.DataFrame) -> None:
    ...
    headers = "".join(f"<th>{html.escape(_human_label(c))}</th>" for c in preview.columns)
    rows = []
...
def render_driver_charts(driver_evidence: dict | None, user_question: str = "") -> bool:
    ...
    for family, payload in list(driver_evidence.get("drivers", {}).items())[:2]:
        ...
        # Focus on decline contributors for a decline question; otherwise show largest absolute movers.
        if (d["delta"] < 0).any():
            d = d[d["delta"] < 0].copy()
        d["impact"] = d["delta"].abs()
        d = d.sort_values("impact", ascending=True).tail(8)

        if d.empty:
            continue

        title = family_labels_ar.get(family, "العوامل الأكثر تأثيرًا") if arabic else family_labels_en.get(family, "Main contributors")
        value_label = "مقدار الانخفاض" if arabic else "Decline amount"
        ...
        if px is not None:
            fig = px.bar(
                ...
                y="name",
                orientation="h",
                text=d["impact"].map(_short_number),
                color_discrete_sequence=["#176BFF"],
                title=title,
            )
            fig.update_traces(textposition="outside", cliponaxis=False)
            fig.update_layout(
                height=max(300, 62 * len(d)),
                margin=dict(l=24, r=48, t=58, b=28),
                paper_bgcolor="#FFFFFF",
                plot_bgcolor="#FFFFFF",
                font=dict(size=12, color="#243B53", family="Segoe UI, Arial"),
                title_font=dict(size=16, color="#0B1F3A"),
                xaxis_title=value_label,
                yaxis_title="",
                showlegend=False,
            )
            fig.update_xaxes(showgrid=True, gridcolor="#E8EEF6", zeroline=False)
...
st.markdown(
    ...
    *{font-family:"Segoe UI",Arial,Tahoma,sans-serif}
    ...
    /* =====================================================
       V6 Brand System + Responsive Layout
       ===================================================== */
    :root{
      --navy:#06182E;
      --navy2:#0B1F3A;
    ...
    /* Use a cleaner enterprise font stack, including Arabic. */
    ...
    /* Home positioning: less unused space, clearer hierarchy. */
    .hero{padding:38px 0 26px!important}
    .hero-logo{width:84px!important}
    .hero h1{font-size:29px!important;color:var(--navy2)!important;letter-spacing:-.35px}
    ...
    .suggestion-slot{
      ...
      box-shadow:0 8px 24px rgba(17,38,73,.045)!important;
      background:#fff!important;
      padding:18px!important;
    }
    .suggestion-slot:hover{
      border-color:#B8D2FF!important;
      box-shadow:0 12px 28px rgba(23,107,255,.09)!important;
    }

    /* Fix the black/unreadable buttons highlighted by the user. */
    ...
    /* Right-panel tools must also be readable. */
    .right-card + div .stButton>button,
    ...
    .right-card{
      border:1px solid var(--line)!important;
      box-shadow:0 8px 24px rgba(17,38,73,.035)!important;
    }

    /* Chat results */
    [data-testid="stChatMessage"]{
      border:1px solid var(--line)!important;
      box-shadow:0 8px 25px rgba(17,38,73,.04)!important;
      padding:.72rem .85rem!important;
    }
    .message-text strong{color:var(--navy2)!important;font-weight:800!important}
    .message-text li{margin-bottom:7px!important}

    /* Chart and supporting data */
    ...
    .supporting-title{
      padding:12px 14px;font-size:12px;font-weight:800;color:var(--navy2);
      border-bottom:1px solid var(--line);background:#FAFCFF;
    }
    .table-scroll{overflow-x:auto;width:100%}
    .nova-table{width:100%;border-collapse:collapse;font-size:12px;color:#243B53}
    .nova-table th{
      text-align:left;background:#F5F8FC;color:#52677D;font-weight:750;
      padding:10px 12px;border-bottom:1px solid var(--line);white-space:nowrap;
    }
    .nova-table td{
      padding:10px 12px;border-bottom:1px solid #EEF2F6;white-space:nowrap;
    }
    .nova-table tr:last-child td{border-bottom:0}
    .nova-table tbody tr:hover{background:#F8FBFF}

    /* Input */
    .ask-label{font-size:13px!important;color:var(--navy2)!important}
    [data-testid="stChatInput"]>div{
      border:1.5px solid #BFD4F5!important;
      min-height:68px!important;
      box-shadow:0 8px 22px rgba(17,38,73,.045)!important;
    }
    [data-testid="stChatInput"]>div:focus-within{
      border-color:var(--blue)!important;
      box-shadow:0 0 0 4px rgba(23,107,255,.08)!important;
    }

    /* Tablet */
    @media(max-width:1024px){
      .block-container{padding-left:.85rem!important;padding-right:.85rem!important}
      .hero{padding:25px 0 19px!important}
      .hero h1{font-size:25px!important}
      .hero-logo{width:72px!important}
      .suggestion-slot{min-height:120px!important}
      .right-card{margin-top:8px!important}
    }
    ...
    @media(max-width:640px){
      ...
      [data-testid="stSidebar"]{
        ...
        min-width:min(88vw,292px)!important;
      }
      .app-header{
        margin:0 0 8px!important;padding:7px 3px 9px!important;
      }
      .header-title{font-size:15px!important}
      .header-actions .status,.header-actions .moon{display:none!important}
      .hero{padding:14px 0 13px!important}
      .hero-logo{width:58px!important}
      ...
      .suggestion-slot{min-height:96px!important;padding:12px!important}
      .suggestion-slot .sicon{
        width:38px!important;height:38px!important;margin-bottom:8px!important
      }
      .stitle{font-size:12px!important}
      .message-text.rtl{
      ...
      .nova-table{font-size:11.5px}
      .nova-table th,.nova-table td{padding:9px 10px}
      .right-card{margin:8px 0!important}
      .security{margin-top:22px!important}
    }
    ...
    /* Give analysis more space on wide laptops/desktops. */
    @media(min-width:1200px){
      [data-testid="stMainBlockContainer"],
      .block-container{
        width:min(95vw,1680px)!important;
      }
      .message-text.rtl,.message-text.ltr{
        max-width:none!important;
      }
      div[data-testid="stPlotlyChart"]{
        min-height:360px;
...
main_col, right_col = st.columns([5.6, 1.15], gap="medium")
    ...
    if not st.session_state.messages:
        ...
        for idx, (icon, cls, q0) in enumerate(cards):
            with prompt_cols[idx]:
                ...
                if st.button("Analyze", key=f"suggested_prompt_{idx}", use_container_width=True):
    ...
    for message in st.session_state.messages:
        with st.chat_message(message["role"]):
            ...
            if df is not None and not df.empty:
                ...
                # For root-cause questions, contribution charts are the primary visual.
                # Show the regular result chart only when no driver evidence exists.
