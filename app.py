...
with main_col:
    ...
    st.markdown(
        '<div class="ask-label"><span class="ask-dot"></span>Ask NovaCore Copilot</div>',
        unsafe_allow_html=True,
    )
    question = st.chat_input("Type your question here, then press the blue arrow...")
    st.markdown(
        '<div class="ask-help">Example: Compare net revenue by year, or اشرح أداء المبيعات حسب السنوات</div>',
        unsafe_allow_html=True,
    )
    if not st.session_state.messages:
