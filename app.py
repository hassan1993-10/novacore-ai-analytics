...
with main_col:
    ...
    st.markdown(f'<div class="chat-tools"><span>{svg_icon("search",19)}</span><span>{svg_icon("table",19)}</span></div>', unsafe_allow_html=True)
    if not st.session_state.messages:
...
with right_col:
    ...
    st.markdown('<div class="right-card"><div class="right-head"><span class="head-icon" style="color:#176BFF">{svg_icon("zap",20)}</span>Quick Actions</div><div class="rbody">', unsafe_allow_html=True)
