...
def _executive_insight(question: str, result: pd.DataFrame, plan: dict) -> str:
...
- Explain trends, concentration, turning points, anomalies or operational implications when supported by the verified result.
- If the result alone cannot prove a root cause, explicitly say it is a hypothesis / يحتاج تحقق and state what additional dimension would validate it.
...
Answer directly. Do not mention implementation details, demo mode, or GitHub Models."""
    ...
    if _needs_deep_insight(question, plan):
        try:
            answer = _executive_insight(question, result, plan)
        except Exception:
