...
def _local_summary(question, result, plan):
    ...
    if time_like:
        ...
        if ar:
            ...
            insights.append(
                f"**أعلى قيمة:** **{_fmt_value(hi[value_col])}** في **{hi[label_col]}**، "
                f"وأقل قيمة **{_fmt_value(lo[value_col])}** في **{lo[label_col]}**."
            )
            if biggest_drop and biggest_drop["delta"] < 0:
                ...
                insights.append(
                    f"**أكبر تراجع بين فترتين:** من **{biggest_drop['from']}** إلى "
                    f"**{biggest_drop['to']}** بمقدار **{_fmt_value(abs(biggest_drop['delta']))}**{pct_txt}."
                )
            if biggest_rise and biggest_rise["delta"] > 0:
                ...
                insights.append(
                    f"**أكبر تحسن بين فترتين:** من **{biggest_rise['from']}** إلى "
                    f"**{biggest_rise['to']}** بمقدار **{_fmt_value(biggest_rise['delta'])}**{pct_txt}."
                )
            ...
            insights.append(
                f"**Peak / low:** **{hi[label_col]}** is highest at **{_fmt_value(hi[value_col])}**; "
                f"**{lo[label_col]}** is lowest at **{_fmt_value(lo[value_col])}**."
            )
            if biggest_drop and biggest_drop["delta"] < 0:
                insights.append(
        ...
        if ar:
            insights.append(
                f"**الأعلى:** **{hi[label_col]}** بقيمة **{_fmt_value(hi[value_col])}**."
            )
            insights.append(
                f"**الأقل:** **{lo[label_col]}** بقيمة **{_fmt_value(lo[value_col])}**."
            )
            if share is not None:
                insights.append(
                    f"**حصة الأعلى:** تمثل **{hi[label_col]}** حوالي **{share:.1f}%** من إجمالي النتائج المعروضة."
                )
            if len(result) >= 3:
            ...
            insights.append(
                f"**Highest:** **{hi[label_col]}** at **{_fmt_value(hi[value_col])}**."
            )
            insights.append(
                f"**Lowest:** **{lo[label_col]}** at **{_fmt_value(lo[value_col])}**."
            )
            if share is not None:
                insights.append(
