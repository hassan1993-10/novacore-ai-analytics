...
try:
    ...
    class _LocalToolRegistry:
        """No-op MCP shim so Streamlit can use analytics functions directly."""
        def tool(self):
            def decorator(func):
                return func
            return decorator
        ...
        def run(self):
            raise RuntimeError(
                "FastMCP is not installed. Install fastmcp to run this module as an MCP server."
            )
...
mcp = FastMCP("novacore-excel-mcp") if MCP_AVAILABLE else _LocalToolRegistry()
